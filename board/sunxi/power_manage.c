...
int axp_get_battery_status(void)
{
	...
	script_parser_fetch(FDT_PATH_CHARGER0, "pmu_ntc_cur", &ntc_cur, 0);
...
#endif
			tick_printf("battery temp (%d) is too high, to be shutdown\n", temp);
			sunxi_bat_temp_handle();
		} else if (temp <= safe_temp[1]) {
#if defined(CONFIG_EINK200_SUNXI) || defined(CONFIG_CMD_SUNXI_BMP)
			sunxi_bmp_display("bat\\bat_ltmp.bmp");
#endif
			tick_printf("battery temp (%d) is too low , to be shutdown\n", temp);
			sunxi_bat_temp_handle();
		}
	}
